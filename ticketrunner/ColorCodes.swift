//
//  ColorCodes.swift
//  ticketrunner
//
//  Created by Luca Kiedrowski on 03.08.17.
//  Copyright © 2017 LucaKiedrowski. All rights reserved.
//

import UIKit

class ColorCodes {
    
    static let ticketrunnerBlue = UIColor(red:0.00, green:0.74, blue:1.00, alpha:1.0)
    static let ticketrunnerYellow = UIColor(red:0.98, green:0.68, blue:0.39, alpha:1.0)
    static let ticketrunnerPurple = UIColor(red:0.72, green:0.55, blue:0.99, alpha:1.0)
    static let ticketrunnerGreen = UIColor(red:0.25, green:0.89, blue:0.56, alpha:1.0)
    static let ticketrunnerRed = UIColor(red:0.95, green:0.36, blue:0.36, alpha:1.0)
    
    static let gradientPurple = UIColor(red:0.95, green:0.02, blue:0.53, alpha:1.0)
    static let lightPurple = UIColor(red:0.73, green:0.76, blue:0.99, alpha:1.0)
    static let darkPurple = UIColor(red:0.34, green:0.43, blue:0.90, alpha:1.0)
    static let leaderboardYellow = UIColor(red:0.98, green:0.76, blue:0.24, alpha:1.0)
    
    static let inactiveElementsGrey = UIColor(red:0.80, green:0.80, blue:0.80, alpha:1.0)
    static let textColorGrey = UIColor(red:0.33, green:0.33, blue:0.33, alpha:1.0)
    
    static let controllerBackground = UIColor(red:0.96, green:0.96, blue:0.97, alpha:1.0)
    
    static let shadowColor = UIColor(red:0.79, green:0.82, blue:0.85, alpha:1.0)
    static let homeYellow = UIColor(red:1.00, green:0.76, blue:0.27, alpha:1.0)
    static let lightGrayText = UIColor(red:0.80, green:0.80, blue:0.80, alpha:1.0)
    
    static let nightModePurple = UIColor(red:0.13, green:0.00, blue:0.28, alpha:1.0)
    static let nightModeLightPurple = UIColor(red:0.24, green:0.19, blue:0.48, alpha:1.0)
    
    static let dayModeBlue = UIColor(red:0.40, green:0.67, blue:0.89, alpha:1.0)
    
}
