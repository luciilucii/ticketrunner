...
protocol MessagesBoxCellDelegate {
    func didSelect()
}

class MessagesBoxCell: BaseCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    lazy var collectionView: UICollectionView = {
        ...
        cv.delegate = self
        cv.dataSource = self
        cv.register(MessagesCell.self, forCellWithReuseIdentifier: cellId)
        cv.backgroundColor = .white
        cv.layer.cornerRadius = 5
        return cv
    }()
    
    var delegate: MessagesBoxCellDelegate?
